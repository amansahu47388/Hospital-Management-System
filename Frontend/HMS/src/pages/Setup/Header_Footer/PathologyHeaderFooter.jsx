...
export default function PathologyHeaderFooter() {
  ...
  const [preview, setPreview] = useState("/uploads/printing/12.jpg");
    ...
    const fetchHeaderData = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setHeaderId(data.id);
          if (data.pathology_bill_header) {
            setPreview(data.pathology_bill_header);
          }
          if (data.pathology_bill_footer) {
            setFooterContent(data.pathology_bill_footer);
          }
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (headerFile) {
        formData.append("pathology_bill_header", headerFile);
      }
      formData.append("pathology_bill_footer", footerContent);

      if (headerId) {
        await updateHeader(headerId, formData);
        notify("success", "Pathology Header & Footer updated successfully");
      } else {
        const response = await createHeader(formData);
        setHeaderId(response.data.id);
        notify("success", "Pathology Header & Footer created successfully");
      }
    } catch (error) {
      console.error("Error saving header data:", error);
      notify("error", "Failed to save Pathology Header & Footer");
    } finally {
      setLoading(false);
    }
  };
