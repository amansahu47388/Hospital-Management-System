...
export default function IpdBillHeaderFooter() {
  const notify = useNotify();
  const [headerId, setHeaderId] = useState(null);
  const [preview, setPreview] = useState("/uploads/printing/4.jpg");
    ...
    const fetchHeaderData = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setHeaderId(data.id);
          if (data.ipd_bill_header) {
            setPreview(data.ipd_bill_header);
          }
          if (data.ipd_bill_footer) {
            setFooterContent(data.ipd_bill_footer);
          }
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (headerFile) {
        formData.append("ipd_bill_header", headerFile);
      }
      formData.append("ipd_bill_footer", footerContent);

      if (headerId) {
        await updateHeader(headerId, formData);
        notify("success", "IPD Bill Header & Footer updated successfully");
      } else {
        const response = await createHeader(formData);
        setHeaderId(response.data.id);
        notify("success", "IPD Bill Header & Footer created successfully");
      }
    } catch (error) {
      console.error("Error saving header data:", error);
      notify("error", "Failed to save IPD Bill Header & Footer");
    } finally {
      setLoading(false);
    }
  };
