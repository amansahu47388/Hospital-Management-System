...
export default function AmbulanceHeaderFooter() {
  ...
  const [preview, setPreview] = useState("/uploads/printing/15.jpg");
    ...
    const fetchHeaderData = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setHeaderId(data.id);
          if (data.ambulance_bill_header) {
            setPreview(data.ambulance_bill_header);
          }
          if (data.ambulance_bill_footer) {
            setFooterContent(data.ambulance_bill_footer);
          }
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (headerFile) {
        formData.append("ambulance_bill_header", headerFile);
      }
      formData.append("ambulance_bill_footer", footerContent);

      if (headerId) {
        await updateHeader(headerId, formData);
        notify("success", "Ambulance Header & Footer updated successfully");
      } else {
        const response = await createHeader(formData);
        setHeaderId(response.data.id);
        notify("success", "Ambulance Header & Footer created successfully");
      }
    } catch (error) {
      console.error("Error saving header data:", error);
      notify("error", "Failed to save Ambulance Header & Footer");
    } finally {
      setLoading(false);
    }
  };
