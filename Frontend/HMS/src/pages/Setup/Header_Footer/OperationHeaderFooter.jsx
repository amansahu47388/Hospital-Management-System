...
export default function OperationHeaderFooter() {
  const notify = useNotify();
  const [headerId, setHeaderId] = useState(null);
  const [preview, setPreview] = useState("/uploads/printing/14.jpg");
    ...
    const fetchHeaderData = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setHeaderId(data.id);
          if (data.operation_header) {
            setPreview(data.operation_header);
          }
          if (data.operation_footer) {
            setFooterContent(data.operation_footer);
          }
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (headerFile) {
        formData.append("operation_header", headerFile);
      }
      formData.append("operation_footer", footerContent);

      if (headerId) {
        await updateHeader(headerId, formData);
        notify("success", "Operation Header & Footer updated successfully");
      } else {
        const response = await createHeader(formData);
        setHeaderId(response.data.id);
        notify("success", "Operation Header & Footer created successfully");
      }
    } catch (error) {
      console.error("Error saving header data:", error);
      notify("error", "Failed to save Operation Header & Footer");
    } finally {
      setLoading(false);
    }
  };
