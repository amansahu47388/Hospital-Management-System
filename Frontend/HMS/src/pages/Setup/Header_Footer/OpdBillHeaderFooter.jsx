...
export default function OpdBillHeaderFooter() {
  const notify = useNotify();
  const [headerId, setHeaderId] = useState(null);
  const [preview, setPreview] = useState("/uploads/printing/2.jpg");
    ...
    const fetchHeaderData = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setHeaderId(data.id);
          if (data.opd_bill_header) {
            setPreview(data.opd_bill_header);
          }
          if (data.opd_bill_footer) {
            setFooterContent(data.opd_bill_footer);
          }
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (headerFile) {
        formData.append("opd_bill_header", headerFile);
      }
      formData.append("opd_bill_footer", footerContent);

      if (headerId) {
        await updateHeader(headerId, formData);
        notify("success", "OPD Bill Header & Footer updated successfully");
      } else {
        const response = await createHeader(formData);
        setHeaderId(response.data.id);
        notify("success", "OPD Bill Header & Footer created successfully");
      }
    } catch (error) {
      console.error("Error saving header data:", error);
      notify("error", "Failed to save OPD Bill Header & Footer");
    } finally {
      setLoading(false);
    }
  };
