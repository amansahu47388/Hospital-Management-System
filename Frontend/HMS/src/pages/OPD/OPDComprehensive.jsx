...
import {Phone, Mail, MapPin, Activity, Droplet, TrendingUp, User, Loader2} from "lucide-react";
import OPDHeader from "../../components/OPDComponent/OPDHeader";
import OPDProfile from "../../components/OPDComponent/OPDProfile";
...
export default function OPDComprehensive() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const opdDetail = opdRes.data;

      let vitals = [];
      try {
        const vitalsRes = await getPatientVitals(opdDetail.patient);
      ...
      const mapped = mapData(opdDetail, vitals);
  ...
  const mapData = (opd, vitals = []) => {
    const p = opd.patient_detail || {};
    const d = opd.doctor_detail || {};
    ...
    return {
      ...
      photo: p.photo,
      symptoms: opd.symptom_name || "No symptoms recorded",
      knownAllergies: [
        ...(p.allergies ? p.allergies.split(",") : []),
        ...(opd.allergies ? opd.allergies.split(",") : [])
      ].map(a => a.trim()).filter((v, i, a) => v && a.indexOf(v) === i),
      tpa: "N/A",
      tpaId: "N/A",
      tpaValidity: "N/A",
      remarks: opd.previous_medical_issue || "No remarks",
      ...
      admission: {
        date: opd.created_at ? new Date(opd.created_at).toLocaleString() : "N/A",
        opdNumber: opd.opd_id ? `OPDN${opd.opd_id}` : "N/A",
        caseId: opd.case_id || "N/A",
        consultant: d.full_name ? `${d.full_name} (${d.id})` : "N/A",
      },
      status: opd.is_discharged ? "Discharged" : "Active",
    };
  };

  useEffect(() => {
    if (opdId) fetchAllData();
  }, [opdId]);
  ...
  const handleProfileSave = async (updatedFormData) => {
    try {
      ...
      // 2. Prepare OPD Specific Info
      const opdFields = {
        previous_medical_issue: updatedFormData.remarks,
        allergies: updatedFormData.knownAllergies, // Also update OPD-specific allergies field
      };

      // 3. Run updates in parallel
      await Promise.all([
        updatePatient(patientData.id, patientFields),
        updateOpdPatient(opdId, opdFields)
      ]);
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen pt-1 md:pt-6 pb-6 bg-gray-50/30">
        {/* Header Section */}
        <OPDHeader patient={patientData} onEditClick={() => setShowProfile(true)} />
