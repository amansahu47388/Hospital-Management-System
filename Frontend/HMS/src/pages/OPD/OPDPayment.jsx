    ...
    return (
        <AdminLayout>
            <div className="min-h-screen bg-gray-50 pb-10">
                ...
                <div className="mx-4 md:mx-6 bg-white rounded-b-lg shadow min-h-[600px] overflow-hidden mt-6">
                    {/* Page Header */}
                            ...
                            <thead className="bg-gray-50 text-gray-600">
                                <tr>
                                    {[
                                        ...
                                        <th key={head} className="px-6 py-2 text-sm bg-gray-300 font-bold">
                                            {head}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {loading && payments.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="py-10 text-center">
                                            <div className="flex items-center justify-center gap-2 text-[#6046B5]">
                                    ...
                                    payments.map((row) => (
                                        <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4 ">TRANID{row.id}</td>
                                            <td className="px-6 py-4">{new Date(row.payment_date).toLocaleDateString()}</td>
                                            <td className="px-6 py-4 ">{row.note || "-"}</td>
                                            <td className="px-6 py-4 ">{row.payment_mode}</td>
                                            <td className="px-6 py-4 ">${Number(row.paid_amount).toFixed(2)}</td>
                                            <td className="px-6 py-4 ">
                                                <div className="flex gap-2 justify-center">
                                                    <button
                                                        onClick={() => handleOpenEdit(row)}
                                                        className="hover:bg-purple-100 text-[#6046B5] p-1.5 rounded-md transition"
                                                        title="Edit"
                                                    >
                                                        <Pencil size={16} />
                                                    </button>
                                                    <button
                                                        ...
                                                        className="hover:bg-red-100 text-red-600 p-1.5 rounded-md transition"
                                                        title="Delete">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                    ...
                                    <tr>
                                        <td colSpan="6" className="py-10 text-center text-gray-500 italic">
                                            {patientId ? "No payment records found." : "Resolving Patient..."}
                                        </td>
                                    </tr>
                                )}
                                {payments.length > 0 && (
                                    <tr className="bg-gray-100 font-bold">
                                        <td colSpan="4" className="px-6 py-4 text-right text-sm">Total Paid Sum :</td>
                                        <td className="px-6 py-4 text-sm text-[#6046B5] font-bold text-lg font-outfit border-collapse ">${totalPaidSum.toFixed(2)}</td>
                                        <td></td>
                                    </tr>
                                )}
                            </tbody>
                        ...
                        <div className="p-6 space-y-4 font-outfit">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <input
                                        type="date"
                                        className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-[#6046B5] outline-none text-sm shadow-sm"
                                        value={formData.payment_date}
                                        onChange={(e) => setFormData({ ...formData, payment_date: e.target.value })}
                                    />
                                </div>
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-bold text-gray-700 uppercase tracking-wide">Amount ($) <span className="text-red-500">*</span></label>
                                    <input
                                        type="number"
                                        className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-[#6046B5] outline-none text-sm font-bold text-[#6046B5] shadow-sm"
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-bold text-gray-700 uppercase tracking-wide">Payment Mode</label>
                                <select
                                    className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-[#6046B5] outline-none text-sm shadow-sm bg-white"
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-bold text-gray-700 uppercase tracking-wide">Note</label>
                                <textarea
                                    className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-[#6046B5] outline-none text-sm min-h-[100px] resize-none shadow-sm pb-10"
