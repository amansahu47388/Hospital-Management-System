                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-gray-100 text-gray-800">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-sm font-bold">Note</th>
                                    <th className="px-6 py-4 text-sm font-bold">Payment Mode</th>
                                    <th className="px-6 py-4 text-sm font-bold">Paid Amount ($)</th>
                                    <th className="px-6 py-4 text-sm font-bold text-center">Action</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white border-b border-gray-200 divide-y divide-gray-100 font-outfit font-medium">
                                {loading && payments.length === 0 ? (
                                    ...
                                    payments.map((row) => (
                                        <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4 text-sm text-gray-900 font-medium font-outfit">TRANID{row.id}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600 font-outfit">{new Date(row.payment_date).toLocaleDateString()}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600 font-outfit truncate max-w-[200px]">{row.note || "-"}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600 font-outfit">{row.payment_mode}</td>
                                            <td className="px-6 py-4 text-sm text-gray-900 font-bold font-outfit">${Number(row.paid_amount).toFixed(2)}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600">
                                                <div className="flex gap-2 justify-center">
                                                    <button
                                                        onClick={() => handleOpenEdit(row)}
                                                        className="hover:bg-purple-200 text-[#6046B5] p-1.5 rounded-md transition shadow-sm"
                                                        title="Edit"
                                                    >
                                                        <Edit2 size={16} />
                                                    </button>
                                                    <button
                                                        ...
                                                        className="hover:bg-red-200 text-red-600 p-1.5 rounded-md transition shadow-sm"
                                                        title="Delete">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                    ...
                                    <tr>
                                        <td colSpan="6" className="py-10 text-center text-gray-500 font-outfit italic">
                                ...
                                {payments.length > 0 && (
                                    <tr className="bg-gray-50 font-bold">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-900 font-bold text-lg font-outfit">${totalPaidSum.toFixed(2)}</td>
                                        <td></td>
                                    </tr>
                                )}
                            </tbody>
                        ...
                        <div className="p-6 space-y-4 font-outfit">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <input
                                        type="number"
                                        className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none text-sm transition-all font-bold text-purple-600"
